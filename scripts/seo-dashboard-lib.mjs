...
export function dailyKeywordBatch(keywords, limit, date = new Date()) {
  if (!keywords.length || limit <= 0) return [];
  const batchSize = Math.min(limit, keywords.length);
  const dayNumber = Math.floor(Date.UTC(date.getUTCFullYear(), date.getUTCMonth(), date.getUTCDate()) / 86400000);
  const start = (dayNumber * batchSize) % keywords.length;
  return Array.from({length: batchSize}, (_, index) => keywords[(start + index) % keywords.length]);
}
...
export function renderDashboard(data) {
  ...
  const workflowLabel = (item) => {
    const scheduledFor = item.workflow?.scheduledFor;
    if (!scheduledFor) return item.workflow?.status || "queued";
    if (scheduledFor <= data.generatedAt.slice(0, 10)) return "published";
    return `scheduled ${new Date(`${scheduledFor}T00:00:00Z`).toLocaleDateString("en-US", {month: "short", day: "numeric", timeZone: "UTC"})}`;
  };
  const queueRows = data.queue.map((item) => `<tr><td><span class="score">${item.score}</span></td><td><b>${escapeHtml(item.title)}</b><small>${escapeHtml(item.keyword)}</small></td><td><span class="pill">${escapeHtml(item.action)}</span></td><td>${escapeHtml(workflowLabel(item))}</td><td>${formatNumber(item.metrics.monthlyVolume)}</td><td>${formatNumber(item.metrics.gscImpressions)}</td><td>${item.metrics.gscPosition ? formatNumber(item.metrics.gscPosition, 1) : "—"}</td></tr>`).join("");
  const emptyQueryMessage = data.status?.gsc?.ok
    ? "Search Console is connected. No queries have impressions in the current reporting window yet."
    : "Connect Search Console to see live queries.";
  const queryRows = (data.gsc?.queries || []).slice(0, 12).map((row) => `<tr><td><b>${escapeHtml(row.keys?.[0] || row.query || "")}</b></td><td>${formatNumber(row.clicks)}</td><td>${formatNumber(row.impressions)}</td><td>${formatNumber(row.position, 1)}</td></tr>`).join("") || `<tr><td colspan="4" class="empty">${emptyQueryMessage}</td></tr>`;
