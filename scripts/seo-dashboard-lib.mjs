...
export function renderDashboard(data) {
  ...
  const queueRows = data.queue.map((item) => `<tr><td><span class="score">${item.score}</span></td><td><b>${escapeHtml(item.title)}</b><small>${escapeHtml(item.keyword)}</small></td><td><span class="pill">${escapeHtml(item.action)}</span></td><td>${formatNumber(item.metrics.monthlyVolume)}</td><td>${formatNumber(item.metrics.gscImpressions)}</td><td>${item.metrics.gscPosition ? formatNumber(item.metrics.gscPosition, 1) : "—"}</td></tr>`).join("");
